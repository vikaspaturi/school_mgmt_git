	...
	public function standard($name)
	{
		switch ($name)
		{
			...
			case 'holidays':
				...
				//$holiday = clone $event;
				//$this->attach($holiday->condition('month', 8)->condition('day', 15)->title('Independence Day')->output('Independence Day'));

				// Attach Labor Day
				//$holiday = clone $event;
				//$this->attach($holiday->condition('month', 5)->condition('day_of_week', 1)->condition('occurrence', 1)->title('Labor Day')->output('Labor Day'));
				...
				// Attach Christmas
				//$holiday = clone $event;
				//$this->attach($holiday->condition('month', 12)->condition('day', 25)->condition('year', 2013)->title('Christmas')->output('Christmas'));
				
				$sql="SELECT * FROM events where status='active'";
				$res = mysql_query($sql);
				//$x= fetch_result($res);
				while($eve=mysql_fetch_array($res))
				{
				$title=$eve['title'];
				$date=$eve['DateTime'];
				...
				$holiday = clone $event;
				$this->attach($holiday->condition('month', $m)->condition('day', $d)->condition('year', $y)->title($title)->output($title));
				}
			break;
			case 'weekends':
				// Weekend events
